...
private:
	...
	/* key: qubit_idx, value: vector of all connected verticies (parities). Length num_qubits */
	std::vector<std::vector<int>> qubit_parities_;
	/* connectivity of vertices (parities). Length num_parity */
			...
			int* col_indices, int* indptr)
	{
		...
		for(size_t q_idx = 0; q_idx < num_qubits; ++q_idx)
		{
			...
			{
				throw std::invalid_argument("Each qubit must appear exactly twice in the parity.");
			}
