...
from union_find import UnionFind3D
...
def num_decoding_failures_noisy_syndromes(L, H, logicals, p, q, num_trials, repetitions):
    ...
    for i in range(num_trials):
        ...
        syndrome_error = (np.random.rand(repetitions, num_stabilisers) < q).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    for L in Ls:
        ...
        log_errors = []
        for p in ps:
            num_errors = num_decoding_failures_noisy_syndromes(L, Hx, logX, p, p, num_trials, L)
